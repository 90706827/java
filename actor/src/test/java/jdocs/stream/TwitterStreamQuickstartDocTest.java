...
public class TwitterStreamQuickstartDocTest extends AbstractJavaTest {
  ...
  static abstract class Example0 {
    //#tweet-source
    Source<Model.Tweet, NotUsed> tweets;
    //#tweet-source
  }
  ...
  public void demonstrateFilterAndMap() {
    ...
    final Source<Model.Author, NotUsed> authors =
      Model.tweets
        .filter(t -> t.hashtags().contains(Model.AKKA))
        .map(t -> t.author);
    //#first-sample
    //#authors-filter-map
    ...
    new Object() {
      //#authors-collect
      JavaPartialFunction<Model.Tweet, Model.Author> collectFunction =
        new JavaPartialFunction<Model.Tweet, Model.Author>() {
          public Model.Author apply(Model.Tweet t, boolean isCheck) {
            if (t.hashtags().contains(Model.AKKA)) {
      ...
      final Source<Model.Author, NotUsed> authors =
        Model.tweets.collect(collectFunction);
      //#authors-collect
    };
  ...
  public void demonstrateMapConcat() {
    //#hashtags-mapConcat
    final Source<Model.Hashtag, NotUsed> hashtags =
      Model.tweets.mapConcat(t -> new ArrayList<Model.Hashtag>(t.hashtags()));
    //#hashtags-mapConcat
  }

  static abstract class HiddenDefinitions {
    //#graph-dsl-broadcast
    Sink<Model.Author, NotUsed> writeAuthors;
    Sink<Model.Hashtag, NotUsed> writeHashtags;
    //#graph-dsl-broadcast
  }

  @Test
  public void demonstrateBroadcast() {
    final Sink<Model.Author, CompletionStage<Done>> writeAuthors = Sink.ignore();
    final Sink<Model.Hashtag, CompletionStage<Done>> writeHashtags = Sink.ignore();

    //#graph-dsl-broadcast
    RunnableGraph.fromGraph(GraphDSL.create(b -> {
      final UniformFanOutShape<Model.Tweet, Model.Tweet> bcast = b.add(Broadcast.create(2));
      final FlowShape<Model.Tweet, Model.Author> toAuthor =
    	  b.add(Flow.of(Model.Tweet.class).map(t -> t.author));
      final FlowShape<Model.Tweet, Model.Hashtag> toTags =
          b.add(Flow.of(Model.Tweet.class).mapConcat(t -> new ArrayList<Model.Hashtag>(t.hashtags())));
      final SinkShape<Model.Author> authors = b.add(writeAuthors);
      final SinkShape<Model.Hashtag> hashtags = b.add(writeHashtags);

      b.from(b.add(Model.tweets)).viaFanOut(bcast).via(toAuthor).to(authors);
  ...
  long slowComputation(Model.Tweet t) {
    try {
  ...
  public void demonstrateCountOnFiniteStream() {
    ...
    final RunnableGraph<CompletionStage<Integer>> counter =
        Model.tweets.map(t -> 1).toMat(sumSink, Keep.right());
    ...
    new Object() {
      ...
      final CompletionStage<Integer> sum = Model.tweets.map(t -> 1).runWith(sumSink, mat);
      //#tweets-fold-count-oneline
    };
  }
  ...
  public void demonstrateMaterializeMultipleTimes() {
    final Source<Model.Tweet, NotUsed> tweetsInMinuteFromNow = Model.tweets; // not really in second, just acting as if
